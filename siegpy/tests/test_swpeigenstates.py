...
siegerts_na_with_grid = SWPBasisSet.from_file(
    filename, nres=5, grid=xgrid, analytic=False
)
...
r_na_k0_3 = Rectangular.from_width_and_center(4.0, 2.5, k0=2.0, h=2.0, grid=xgrid)
g_na_1 = Gaussian(0.25, 0.0, h=h, grid=xgrid)
g_na_k0_1 = Gaussian(0.25, 0.0, k0=2.0, h=h, grid=xgrid)
g_na_2 = Gaussian(0.25, 0.2, h=h, grid=xgrid)
g_na_k0_2 = Gaussian(0.25, 0.2, k0=2.0, h=h, grid=xgrid)
...
class TestSWPSiegert:
    ...
    @pytest.mark.parametrize(
        "value, expected", [(bnd1.parity, "e"), (bnd2.parity, "o")]
    )
    ...
    @pytest.mark.parametrize("ks", [1 + 1.0j, -1 + 1.0j, 0.0j])
    ...
    @pytest.mark.parametrize(
        "value, expected",
        [
            (res_na_1.scal_prod(r_na_1), 0.0j),
            (res_na_2.scal_prod(r_na_1), 4.1743949112633345 - 2.607164369836998j),
            (res_na_1.scal_prod(r_na_k0_1), 0.70717092799511272 - 1.7207880711569299j),
            (res_na_1.scal_prod(r_na_2), -0.47198718301390397 - 0.83427379754553099j),
            (res_na_2.scal_prod(r_na_2), 1.3505045842605954 - 0.49831687689635684j),
            (res_na_1.scal_prod(r_na_k0_2), -0.47211403485904602 - 1.7626664365405986j),
            (res_na_1.scal_prod(g_na_1), 0.0j),
            ...
            (res_na_2.scal_prod(g_na_2), 0.98439751748650228 - 0.065913082743066342j),
            (
...
class TestSWPContinuum:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (c_e_na.scal_prod(r_na_k0_1), -0.52973455183891471 - 0.63879226854430993j),
            ...
            (c_e_na.scal_prod(r_na_k0_3), 0.0052031824473088156 - 0.94598903180912952j),
            (c_e_na.scal_prod(g_na_1), -0.22338282414061131 - 0.26937117937892679j),
            (c_o_na.scal_prod(g_na_1), 0.0j),
            (c_e_na.scal_prod(g_na_k0_1), -0.22341030109728754 - 0.26940431308226365j),
            ...
            (c_e_na.scal_prod(g_na_k0_2), -0.16248453420586526 - 0.19555059652396745j),
        ],
    )
    def test_numerical_scal_prod(self, value, expected):
        decimal = 14
