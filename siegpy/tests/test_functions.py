...
r_grid_expected = np.array([0.0, 0.0, 0.0, 2.0, 2.0, 2.0, 0.0, 0.0, 0.0], dtype=complex)
...
class TestGaussian:
    @pytest.mark.parametrize(
        "to_evaluate",
        ["Gaussian(0, 0, h=1)", "Gaussian(-2, -2, h=1)"],  # sigma = 0  # sigma < 0
    )
...
class TestRectangular:
    ...
    def test_conjugate(self):
        assert r_momentum.conjugate() == Rectangular(
            xc - a / 2.0, xc + a / 2, k0=-k0, h=h
        )
