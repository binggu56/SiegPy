...
import pytest
import numpy as np
from siegpy import (SWPotential, SWPContinuum, Rectangular, Gaussian,
                    Eigenstate, Potential, SWPBasisSet)
from siegpy.basisset import BasisSetError, _set_mat_space  # noqa: F401
from siegpy.functions import Function
from siegpy.analyticbasisset import (_MLE,  # noqa F401
                                     _set_mat_space_S,
                                     _set_mat_space_c)
...
class TestSWPBasisSet():
    ...
    @pytest.mark.parametrize("state, wavenumber", [
        (bnds[0], 0. + 4.42578048382546j),
        (abnds[0], 0. - 4.402395642496243j),
    ...
    @pytest.mark.parametrize("basis, length", [
        (siegerts, 22), (bnds, 7), (abnds, 5),
        (res, 5), (ares, 5), (cont, 0)
    ])
    ...
    @pytest.mark.parametrize("basis", [
        bnds.bounds, siegerts.bounds, bnds_grid
    ])
    ...
    def test_find_Siegert_states_bounds_only(self):
        ...
        s = SWPBasisSet.find_Siegert_states(p, 0.0, 0.0, 0.0,
                                            bounds_only=True)
        assert s == s.bounds
    ...
    @pytest.mark.parametrize("analytic, other_analytic", [
        (True, False), (False, True)
    ])
    def test_analytic_raises_ValueError_different_grid(self, analytic,
                                                       other_analytic):
    ...
    def test_scal_prod(self):
        sp_bnds_expected = np.array(
            [1.177008+0.j, 0.300794+0.j, 0.551730-0.j, 0.163651-0.j,
             -0.010483+0.j, -0.158772+0.j, -0.112085-0.j])
        np.testing.assert_array_almost_equal(bnds_grid.scal_prod(r),
                                             sp_bnds_expected)

    def test_MLE_contributions_to_CR(self):
        # Test function with an initial momentum
        test = Gaussian(r.width/10, r.center, k0=1.)
        expected = np.array(
            [0.133479-9.787129e-18j, 0.013258-2.446782e-18j,
             0.094299+2.446782e-18j, 0.038485+7.340347e-18j,
             0.045363+0.000000e+00j, 0.049518+7.340347e-18j,
    ...
    def test_MLE_contributions_to_zero_operator(self):
        ...
        test = Gaussian(r.width/10, r.center, k0=1.)
        expected = np.array(
            [-2.211391e-18-0.03016j, -5.711330e-19-0.003095j,
             6.058881e-19-0.023351j, 1.999759e-18-0.010485j,
             0.000000e+00-0.014424j, 3.087505e-18-0.020828j,
    ...
    @pytest.mark.parametrize("method, expected", [
        ("MLE_completeness_convergence", ([0.], [0.4612232])),
        ("Berggren_completeness_convergence", ([0.], [2*0.4612232])),
        ("MLE_zero_operator_convergence", ([0.], [0.-0.1098721j]))
    ])
    ...
    def test_exact_completeness_convergence(self):
        expected = [0., 1., 2.], [0.9224464, 0.9237561, 0.9359722]
        expected_even = [0., 1., 2.], [0.9335314, 0.934965, 0.9385559]
        ...
        CR_conv_even = siegerts.exact_completeness_convergence(
            r_even, hk=1, kmax=2)
        np.testing.assert_almost_equal(expected, CR_conv)
        np.testing.assert_almost_equal(expected_even, CR_conv_even)
...
#                                       eval("basis.{}(r)".format(method)))
    ...
    def test_MLE_strength_function(self):
        ...
        expected = \
            [0.0011176, 0.0097039, 0.0142134, 0.0163033, 0.0167877, 0.016375]
        np.testing.assert_almost_equal(
            bnds_grid.MLE_strength_function(r, kgrid), expected)

    @pytest.mark.parametrize("test_func, expected", [
        (r, [0.0006824, 0.0060233, 0.0234454,
             0.0039414, 0.0004156, 0.0015201]),
        (r_even, [0.001344, 0.0055976, 0.0046848,
                  0.0069373, 0.0003513, 0.0017803])
    ])
    def test_exact_strength_function_OTF(self, test_func, expected):
        np.testing.assert_almost_equal(
            bnds_grid.exact_strength_function_OTF(
                test_func, kgrid, hk=10**(-2), eta=10**(-2)), expected)

    @pytest.mark.parametrize("test_func, expected", [
        (r, [0.0008341, 0.0008478, 0.0008561,
             0.0007844, 0.000595, 0.0003791]),
        (r_even, [0.0003142, 0.0003172, 0.0003147,
                  0.0002813, 0.0002097, 0.0001333])
    ])
    ...
    def test_MLE_propagation(self):
        # Test that the result is almost equal to its expected value.
        expected = \
            [[0.160550+0.j, -0.153228+0.j, 0.994785+0.j, 0.855366+0.j,
              -0.065969+0.j],
             [4.258355-4.229955j, -0.658954+3.347923j, -0.976769+0.216613j,
              -0.370865-3.831607j, -3.690687+3.539654j]]
        np.testing.assert_array_almost_equal(
            expected,
            siegerts_grid.MLE_propagation(r, time_grid)
        )

    def test_Berggren_propagation(self):
        # Test that the result is almost equal to its expected value.
        expected = \
            [[0.120956+0.056237j, -0.182397-0.031712j, 1.005887+0.0057j,
              0.886923-0.009412j, -0.098459-0.154117j],
             [0.062060-0.063997j, -0.228571-0.438164j, -0.860815+0.034619j,
              -0.733676-0.356122j, -0.077606-0.238134j]]
        np.testing.assert_array_almost_equal(
            siegerts_grid.Berggren_propagation(r, time_grid), expected)

    @pytest.mark.parametrize("w, expected_method", [
        ({'b': 1, 'r': 1}, "Berggren_propagation"),
        (None, "exact_Siegert_propagation")
    ])
    def test_Siegert_propagation(self, w, expected_method):
        np.testing.assert_array_almost_equal(
            siegerts_grid.Siegert_propagation(
                r, time_grid, weights=w),
    ...
    def test_exact_Siegert_propagation_values(self):
        ...
        expected = \
            [[0.160550+0.j, -0.153228+0.j, 0.994785+0.j, 0.855366+0.j,
              -0.065969+0.j],
             [0.070821-0.072076j, -0.224232-0.431373j, -0.860860+0.038198j,
              -0.739404-0.356705j, -0.086283-0.232656j]]
        np.testing.assert_array_almost_equal(
    ...
    @pytest.mark.parametrize("to_evaluate", [
        "siegerts_grid._propagate(r, time_grid)",  # missing weigths
    ])
