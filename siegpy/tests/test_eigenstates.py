...
wf_complex = Eigenstate([1, 2, 3], [1+1.j, 2.j, 1+2.j], 2.0)
wf_complex_conj = Eigenstate([1, 2, 3], [1-1.j, -2.j, 1-2.j], 2.0)
...
class TestEigenstate():
    ...
    def test_init_raises_ValueError(self):
        ...
        cplx_grid = np.array([1.j]*len(wf_expected))
        with pytest.raises(ValueError):
            Eigenstate(cplx_grid, wf_expected, 1.0)
        # The grid is real, this works fine
        cplx_grid = np.array([0.j]*len(wf_expected))
        Eigenstate(cplx_grid, wf_expected, 1.0)
        # the grid is not real
