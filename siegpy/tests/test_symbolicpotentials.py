...
tgp_fg = TwoGaussianPotential.from_Gaussians(
    Gaussian(1, 10, h=4), Gaussian(1, -10, h=4)
)
fgp = FourGaussianPotential(1, 20, 0, 1, 10, 4, 1, -10, 4, 1, -20, 0)
fgp_fg = FourGaussianPotential.from_Gaussians(
    Gaussian(1, 20, h=0),
    Gaussian(1, 10, h=4),
    Gaussian(1, -10, h=4),
    Gaussian(1, -20, h=0),
...
class TestWoodsSaxonPotential:
    ...
    @pytest.mark.parametrize(
        "method, value", [("width", 1), ("depth", 10), ("sharpness", 4)]
    )
...
class TestGaussianPotentials:
    @pytest.mark.parametrize("obj1, obj2", [(tgp, fgp), (tgp, tgp_fg), (fgp, fgp_fg)])
    ...
    @pytest.mark.parametrize(
        "method, value",
        [("sigmas", [1, 1]), ("centers", [10, -10]), ("amplitudes", [4, 4])],
    )
