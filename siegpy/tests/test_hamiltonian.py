...
class TestHamiltonian:
    ...
    def test_solve_1(self):
        ...
        np.testing.assert_array_almost_equal(
            np.array(basis.energies), expected_energies
        )
        ...
        np.testing.assert_array_almost_equal(np.array(basis.virials), expected_virials)
        # Test that a symbolic SWPotential gives the same results
        sym_func = sympy.Piecewise((0, x < -l / 2), (-V0, x <= l / 2), (0, x > l / 2))
        sym_pot = SymbolicPotential(sym_func, grid=xgrid)
        ham = Hamiltonian(sym_pot, cm)
        sym_basis = ham.solve()
        np.testing.assert_array_almost_equal(
            np.array(sym_basis.energies), expected_energies
        )
        np.testing.assert_array_almost_equal(
            np.array(sym_basis.virials), expected_virials
        )
    ...
    def test_solve_continuum(self):
        ...
        expected_energies = np.array(
            [
                (-9.8288286161968976 + 0j),
                (0.18617777752368994 + 0j),
                (0.18617792287242849 + 0j),
                (-9.3383898863702761 + 0j),
                (0.71296945196866168 + 0j),
                (0.7129697782275819 + 0j),
                (-8.595473591590606 + 0j),
                (1.4930119121801471 + 0j),
                (1.4930122268258661 + 0j),
                (-7.7035348937163128 + 0j),
                (2.4021424682202617 + 0j),
                (2.4021426699256252 + 0j),
                (-6.7916276637316777 + 0j),
                (3.3011897225301228 + 0j),
                (3.3011898226690479 + 0j),
                (-5.9996161956567011 + 0j),
                (4.0565460098448725 + 0j),
                (4.0565460483764415 + 0j),
                (-5.4587483341975407 + 0j),
                (4.5581426867299424 + 0j),
        ...
        expected_virials = np.array(
            [
                (0.068957671136551296 + 0j),
                (0.081706100499458409 + 0j),
                (0.081706716674626631 + 0j),
                (0.26737626672232179 + 0j),
                (0.31078504439895654 + 0j),
                (0.31078634874138406 + 0j),
                (0.57066555202735969 + 0j),
                (0.64535082474379568 + 0j),
                (0.64535198905296198 + 0j),
                (0.9401929123726398 + 0j),
                (1.0300394704547271 + 0j),
                (1.0300401604090617 + 0j),
                (1.3261035947786457 + 0j),
                (1.4063693516031113 + 0j),
                (1.4063696715986373 + 0j),
                (1.6704586014440586 + 0j),
                (1.7201610923651163 + 0j),
                (1.7201612093549419 + 0j),
                (1.9122967253198124 + 0j),
                (1.9275603353958155 + 0j),
